...
const relativeList = "scripts/cpanel-fan-suite-20260907c-release.txt";
...
const allowed = new Set([...rootPages, ...toolFiles, ...releaseFiles, "core.js", "sw.js",
...
assert.match(fs.readFileSync(path.join(root, "core.js"), "utf8"), /PRODUCT_ASSET_VERSION = ['"]20260907c['"]/);
assert.match(fs.readFileSync(path.join(root, "sw.js"), "utf8"), /dj-house-v2026-09-07-3/);
for (const file of [...toolFiles, "lineup-lab/index.html"].filter(n => n.endsWith(".html"))) {
  const html = fs.readFileSync(path.join(root, file), "utf8");
  assert.match(html, /court-themed/);
  assert.match(html, /basketball-theme\.css\?v=20260907c/);
  assert.ok(!html.includes("?v=20260907b"), "Stale cache reference: " + file);
}
if (process.argv.includes("--live")) {
  ...
  await Promise.all(Array.from({ length: 4 }, async () => {
    while (pending.length) {
      ...
      const response = await fetch("https://djshouseofcards-comics.com/" + row.file + "?releaseAudit=20260907c-" + Date.now(), {
