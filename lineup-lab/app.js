...
import {
  ...
  parsePlayerCsv,
  validateDataset,
} from "./player-data.js?v=20260905c";
import {
  ...
  nbaSeasonLabel,
} from "./supabase-nba-data.js?v=20260905c";
import {
  derivePlayerRateViews,
  explainOptimizationSelection,
} from "./fan-analytics.js?v=20260905c";
import {
  buildOpponentGamePlan,
} from "./opponent-gameplan.js?v=20260905c";
import {
  decodeScenarioQuery,
  encodeScenarioQuery,
} from "./scenario-url.js?v=20260905c";
import { pruneLineupLabDatasetCache } from "./lineup-cache.js?v=20260905c";
...
const FIXTURE_URL = "./fixtures/timberwolves-2021-22.json?v=20260905c";
const OPTIMIZER_WORKER_URL = new URL("./optimizer-worker.js?v=20260905c", import.meta.url);
...
function renderExactObjectiveReasons(player, result, insight) {
  ...
  if (entries.length) {
    entries.slice(0, 3).forEach(([metric, item]) => {
      ...
      row.textContent = `${resultMetricLabel(metric)}: ${formatOrdinal(percentile * 100)} ${percentileContext}.`;
      list.append(row);
    });
  } else {
  ...
  if (result.best?.rotation && rateEvidence?.applied) {
    ...
    strip.append(resultEvidenceItem(
      ...
      assignedRoleScoring?.applied
        ? rateEvidence.workloadCalibration ? "Held-out calibrated rates + workload" : "Sample-adjusted rates + workload assumptions"
...
function syncModelChoice() {
    ...
    if (riskHelp) riskHelp.textContent = scout
      ...
      : fittedSeason
        ? "For 2025–26 regular season, fitted sample corrections replace the Reliable/Upside adjustments for standard box-score rates. This selector still controls uncalibrated impact/context assumptions; it never changes minute limits."
        : "Reliable is more cautious with uncertain rates. Upside trusts emerging production sooner. These are uncalibrated sensitivity settings for this source, not accuracy guarantees or minute limits.";
