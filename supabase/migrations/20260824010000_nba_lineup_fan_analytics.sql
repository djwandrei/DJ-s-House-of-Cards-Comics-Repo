-- Fan-grade, read-only context for the Lineup Lab.
--
-- The original player-pool view deliberately exposed only enough fields for
-- per-game optimization.  This later definition preserves that contract and
-- adds narrow, source-backed context for presentation: raw stint totals,
-- Basketball Reference advanced metrics, an estimated team-possession
-- denominator, and a same-season/phase league baseline.
--
-- Important: `estimated_team_possessions` is the conventional
-- FGA + 0.44 * FTA - ORB + TOV estimate built from the selected team's own
-- totals. Individual exposure divides a player's minutes by team total minutes
-- divided by five (the five simultaneous player slots). It is useful for a
-- clearly labelled estimated per-100 view, but it is not a substitute for an
-- imported play-by-play possession count. The
-- browser must retain that qualifier in all fan-facing copy.
...
-- deployments ran the earlier hardening migration, but revoking explicitly
-- here keeps this self-contained definition read-only if it is applied on a
-- restored database or a partially migrated environment.
...
comment on view public.nba_lineup_player_pool is
  'Read-only team-stint pool for Lineup Lab, including source-backed fan analytics context. Per-100 views must be labelled estimated unless an exact possession source is added.';
