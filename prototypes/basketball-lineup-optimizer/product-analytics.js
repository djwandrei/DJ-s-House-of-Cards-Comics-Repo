...
import { readPlayerUsage } from "./player-projection.js?v=__LINEUP_LAB_ASSET_VERSION__";
import { evaluateIndividualPlayers } from "./individual-player-evaluation.js?v=__LINEUP_LAB_ASSET_VERSION__";
...
function rankValues(records, accessor, { lowerIsBetter = false } = {}) {
  ...
  while (start < entries.length) {
    ...
    for (let index = start; index <= end; index += 1) result.set(entries[index].id, score);
    start = end + 1;
  }
  return result;
}
...
function sourceRecord(player, roleRecord, index) {
  ...
  return {
    ...
    raw: {
      ...
      perimeterVolume: nonNegative(roleRecord?.values?.threePointVolume),
      freeThrowPressureProxy,
      playmaking: finite(roleRecord?.values?.assistPct) ?? finite(roleRecord?.values?.playmaking),
      ...
      shotBlocking: finite(roleRecord?.values?.blockPct) ?? nonNegative(roleRecord?.values?.blocks),
      rebounding: finite(roleRecord?.values?.totalReboundPct) ?? nonNegative(roleRecord?.values?.rebounding),
    },
    bases: {
      scoringEfficiency: trueShootingPct !== null ? "reported true-shooting percentage" : "effective field-goal percentage",
      playmaking: Number.isFinite(roleRecord?.values?.assistPct) ? "reported assist percentage" : "assists per 36 minutes",
      perimeterVolume: roleRecord?.values?.threePointVolumeUnit || "unavailable",
      shotBlocking: Number.isFinite(roleRecord?.values?.blockPct) ? "reported block percentage" : "blocks per 36 minutes",
      rebounding: Number.isFinite(roleRecord?.values?.totalReboundPct) ? "reported total rebound percentage" : "rebounds per 36 minutes",
    },
  };
}

function dimension(id, score, rawValue, unit, basis, evidenceMode, confidence, reason = null) {
  const definition = SKILL_DIMENSIONS[id];
  const available = Number.isFinite(score) && Number.isFinite(rawValue);
  return {
    id,
    ...
    rawValue: available ? round(rawValue, 5) : null,
    unit: available ? unit : null,
    basis: available ? basis : null,
    evidenceMode,
    confidence: available ? confidence : "unavailable",
    reason: available ? null : reason || "Required source evidence is unavailable.",
  };
}
...
export function buildPlayerSkillProfiles(players, options = {}) {
  ...
  const records = sourceRecords.filter((record) => selectedIds.has(record.id)).map((record) => {
    ...
    const getRank = (id) => ranks[id].get(record.id) ?? null;
    ...
    const dimensions = {
      scoringVolume: dimension("scoringVolume", getRank("scoringVolume"), raw.scoringVolume, "points per 36", "box-score rate", "direct-box-score", confidence),
      scoringEfficiency: dimension("scoringEfficiency", getRank("scoringEfficiency"), raw.scoringEfficiency, "share", record.bases.scoringEfficiency, "direct-box-score", confidence),
      perimeterAccuracy: dimension("perimeterAccuracy", getRank("perimeterAccuracy"), raw.perimeterAccuracy, "share", "reported three-point percentage", "direct-box-score", confidence),
      freeThrowPressureProxy: dimension("freeThrowPressureProxy", getRank("freeThrowPressureProxy"), raw.freeThrowPressureProxy, "FTA per FGA", "reported free-throw attempts divided by field-goal attempts", "box-score-proxy", confidence, "Attempt totals are unavailable."),
      playmaking: dimension("playmaking", getRank("playmaking"), raw.playmaking, Number.isFinite(record.roleRecord?.values?.assistPct) ? "share" : "assists per 36", record.bases.playmaking, "direct-box-score", confidence),
      usageLoad: dimension("usageLoad", getRank("usageLoad"), raw.usageLoad, "share", "reported usage percentage", "advanced-box-score", confidence, "Reported usage percentage is unavailable."),
      ballSecurity: dimension("ballSecurity", getRank("ballSecurity"), raw.ballSecurity, "turnovers per 36", "lower turnover rate ranks higher", "box-score-proxy", confidence),
      spacingProxy: dimension("spacingProxy", spacingScore, raw.perimeterVolume, record.bases.perimeterVolume, "three-point accuracy and verified attempt volume", "box-score-proxy", confidence, "Both three-point accuracy and attempt volume are required."),
      defensiveActivity: dimension("defensiveActivity", getRank("defensiveActivity"), raw.defensiveActivity, "steals plus blocks per 36", "box-score defensive events", "box-score-proxy", confidence),
      shotBlocking: dimension("shotBlocking", getRank("shotBlocking"), raw.shotBlocking, Number.isFinite(record.roleRecord?.values?.blockPct) ? "share" : "blocks per 36", record.bases.shotBlocking, "box-score-proxy", confidence),
      rebounding: dimension("rebounding", getRank("rebounding"), raw.rebounding, Number.isFinite(record.roleRecord?.values?.totalReboundPct) ? "share" : "rebounds per 36", record.bases.rebounding, "direct-box-score", confidence),
    };
...
function validatedObservedSynergy(evidence, selectedIds) {
  ...
  if (
    ...
    return {
      ...
      reason: "Observed synergy requires validated provenance, independent box-score reconciliation, and a model version.",
    };
  }
  ...
  if (evidence.alreadyShrunk === true && evidence.publishable === true && alreadyShrunk !== null) {
    return {
      ...
      reliability: finite(evidence.reliability),
...
function adjustedImpact(evidence) {
  ...
  return {
    ...
    modelVersion: String(evidence.modelVersion || "unspecified-validated-model"),
...
export function buildPlayerValueProfiles(players, options = {}) {
  ...
  const records = selected.map((player, index) => {
    ...
    return {
      ...
      impact: {
        boxScoreModel: {
          available: Boolean(individualEvaluation?.coverage?.advancedBoxMetricCount),
          metrics: individualEvaluation?.metrics || null,
          uncertainty: null,
          reason: individualEvaluation?.coverage?.advancedBoxMetricCount
...
export function assessBasketballProductReadiness(options = {}) {
  ...
  const records = [
    ...
    {
      id: "chemistry",
      status: chemistry?.observedSynergy?.available ? "observed-and-proxy" : chemistry ? "proxy-only" : "needs-input",
      available: Boolean(chemistry),
      reason: chemistry?.observedSynergy?.available
    ...
    {
      id: "playerImpact",
      status: options.impactEvidenceById ? "partial" : "box-model-only",
